...
function upclick(params)
{
    ...
    container.setAttribute("id", params['setId']);
    //container.onmouseover = function() {changePositionData()};
    //container.onmouseout = function() {changePositionDataback()};
    // frame -> div
    var frame_name = 'frame' + new Date().getTime().toString().substr(8);

    // IE require such creation method
    container.innerHTML =
        '<iframe id="upload_imagform" name="'+frame_name+'" src="about:blank" onload="this.onload_callback()"></iframe>';
    ...
    frame.onload_callback =
        function()
        {
            ...
            if (params['maxsize'])
            {
               ...
               // container.onmouseover = "changePositionData()";
